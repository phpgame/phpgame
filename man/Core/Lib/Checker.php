...
class Checker
{
    ...
    public static function checkExtension()
    {
        ...
        foreach($need_map as $ext_name=>$must_required)
        {
            ...
            {
                \Man\Core\Master::notice($ext_name. " [NOT SUPORT BUT REQUIRED] \tYou have to compile CLI version of PHP with --enable-{$ext_name} \tPHPGame start fail");
                exit('* ' . $ext_name. " \033[31;40m [NOT SUPORT BUT REQUIRED] \033[0m\n\n\033[31;40mYou have to compile CLI version of PHP with --enable-{$ext_name} \033[0m\n\n\033[31;40mPHPGame start fail\033[0m\n\n");
            }
    ...
    public static function checkDisableFunction()
    {
        ...
        foreach($check_func_map as $func)
        {
            ...
            {
                \Man\Core\Master::notice("Function $func may be disabled\tPlease check disable_functions in php.ini \t PHPGame start fail");
    ...
    public static function checkWorkersConfig()
    {
        ...
        foreach (Config::getAllWorkers() as $worker_name=>$config)
        {
            if(isset($config['user']))
            {
                ...
                if(!self::checkWorkerUserName($worker_user))
                {
                    ...
                    \Man\Core\Master::notice("Can not run $worker_name processes as user $worker_user , User $worker_user not exists\tPHPGame start fail");
            ...
            {
                \Man\Core\Master::notice(str_pad($worker_name, 40)." [start_workers not set]\tPHPGame start fail");
        ...
        {
            \Man\Core\Master::notice("Number of worker processes can not be greater than " . \Man\Core\Master::SERVER_MAX_WORKER_COUNT . ".\tPlease check start_workers in " . WORKERMAN_ROOT_DIR . "config/main.php\tPHPGame start fail");
    ...
    public static function checkPidFile()
    {
        ...
        {
            \Man\Core\Master::notice("\033[33;40mPHPGame already started\033[0m", true);
            exit;
        }
        
        if(is_dir(WORKERMAN_PID_FILE))
        {
            exit("\n\033[31;40mpid-file ".WORKERMAN_PID_FILE." is Directory\033[0m\n\n\033[31;40mPHPGame start failed\033[0m\n\n");
        }
